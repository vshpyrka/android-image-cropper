...
fun ImageCropper(
    ...
    BoxWithConstraints(modifier = modifier.fillMaxSize()) {
        ...
        // State
        var imageSize by remember(bitmap) { mutableStateOf(Size(bitmap.width.toFloat(), bitmap.height.toFloat())) }
        ...
        Box(
                ...
                .pointerInput(Unit) {
                    detectDragGestures(
                        ...
                        onDrag = { change, dragAmount ->
                            ...
                            when (handle) {
                                Handle.Center -> {
                                    val newLeft = (r.left + totalDeltaImage.x).coerceIn(0f, imageSize.width - r.width)
                                    val newTop = (r.top + totalDeltaImage.y).coerceIn(0f, imageSize.height - r.height)
            ...
            Canvas(modifier = Modifier.fillMaxSize()) {
                ...
                // Draw Overlay
                // Top
                drawRect(Color.Black.copy(alpha = 0.5f), topLeft = Offset(0f, 0f), size = Size(size.width, screenCropRect.top))
                // Bottom
                drawRect(Color.Black.copy(alpha = 0.5f), topLeft = Offset(0f, screenCropRect.bottom), size = Size(size.width, size.height - screenCropRect.bottom))
                // Left (middle)
                drawRect(Color.Black.copy(alpha = 0.5f), topLeft = Offset(0f, screenCropRect.top), size = Size(screenCropRect.left, screenCropRect.height))
                // Right (middle)
                drawRect(Color.Black.copy(alpha = 0.5f), topLeft = Offset(screenCropRect.right, screenCropRect.top), size = Size(size.width - screenCropRect.right, screenCropRect.height))
            ...
            fun HandleBox(handle: Handle, position: Offset) {
                Box(
                    modifier = Modifier
                        .size(testHandleSize)
                        .offset { IntOffset((position.x - testHandleHalf.toPx()).roundToInt(), (position.y - testHandleHalf.toPx()).roundToInt()) }
            ...
            // Crop Rect Center for dragging
             Box(
                modifier = Modifier
                    .size(width = with(density) { screenCropRect.width.toDp() }, height = with(density) { screenCropRect.height.toDp() })
                    .offset { IntOffset(screenCropRect.left.roundToInt(), screenCropRect.top.roundToInt()) }
                    .testTag("CropRect")
            )
        }
    }
